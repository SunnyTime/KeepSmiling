    ...
    public static void displayImage(final String url, GifImageView gifView, TextView tvProgress, int displayWidth) {
        ...
        startDownLoad(url, new File(cacheFile.getAbsolutePath() + ".tmp"), new DownLoadTask() {
            ...
            public void onLoading(long total, long current) {
                ...
                Logger.t("AlexGIF").i("下载gif的进度是" + progress + "%" + "    现在大小" + current + "   总大小" + total);
                ArrayList<ProgressViews> viewses = memoryCache.get(url);
                if (viewses == null) return;
                Logger.t("AlexGIF").i("该gif的请求数量是" + viewses.size());
